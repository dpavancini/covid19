...
dados_selecionados %>% 
  ...
  scale_y_log10()  + theme_minimal() + labs(title = paste("Evolução após 1º caso, Covid-19 - ",format(Sys.time(), "%d/%m/%Y")), caption = "Em escala logaritmica.Os dados são atualizados diariamente.\n Fonte:https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide") + xlab("Dias desde 1º caso") + ylab("Casos confirmados") +
...
theme(legend.position="bottom",plot.title = element_text(hjust = 0.5)) +geom_label_repel(check_overlap = TRUE, data = dados_rotulos,show.legend = FALSE)
...
dados_selecionados_estat %>% 
  ...
  ggplot(aes(x=GeoId, y=med_beds, label=GeoId)) + geom_bar(aes(y=med_bed_need_est,fill ="med_bed_need_est"),fill="red",stat="identity",alpha=1) + geom_bar(stat='identity', alpha = 0.5)+ theme_minimal() + labs(title = paste("Total de Leitos e Ocupação de Leitos estimada/1000 habitantes - ",format(Sys.time(), "%d/%m/%Y")), caption = "Considerando uma taxa de hospitalização de 20% dos casos confirmados. Os dados são atualizados diariamente.\n Fonte:https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide") + xlab("País") + ylab("Leitos por 1000 habitantes")+
...
casos_por_estado %>% 
  ...
  scale_y_log10()  + theme_minimal() + labs(title = paste("Evolução por estado - ",format(Sys.time(), "%d/%m/%Y")), caption = "Em escala logaritmica. Apenas estados com mais de 10 casos.Os dados são atualizados diariamente.\n Fonte:https://raw.githubusercontent.com/wcota/covid19br/master/cases-brazil-states.csv") + xlab("Dias desde 10º caso") + ylab("Casos confirmados")+
...
plot <- casos_por_estado %>% 
  ...
  pivot_longer(names_to = 'indicador',cols =      c('newCases','totalCases','g_new_cases','g_total_cases'))%>% 
  ggplot(aes(x=days_from_first, y=value)) + geom_bar(aes(x=days_from_first,y=value, fill=indicador), data= . %>% filter(indicador %in% c('newCases','totalCases')),stat='identity', position = 'dodge')+
theme_minimal() + labs(title = paste("Evolução em",uf," - ",format(Sys.time(), "%d/%m/%Y")), caption = "Em escala logaritmica. Os dados são atualizados diariamente.\n Fonte:https://raw.githubusercontent.com/wcota/covid19br/master/cases-brazil-states.csv") + xlab("Dias desde 1º caso") + ylab("Casos confirmados")+ 
  scale_fill_manual(name="Indicador", breaks = c("newCases", "totalCases"), 
                    labels = c("Novos casos", "Casos Acumulados"), 
                    values = c("#ff6a6a","grey"))+
theme(legend.position="bottom",
