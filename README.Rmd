...
dados_selecionados_estat %>% 
  ...
  ggplot(aes(x=GeoId, y=med_beds, label=GeoId)) + geom_bar(aes(y=med_bed_need_est,fill ="med_bed_need_est"),fill="red",stat="identity",alpha=1) + geom_bar(stat='identity', alpha = 0.5)+ theme_minimal() + labs(title = paste("Total de Leitos e Ocupação de Leitos estimada/habitantes - ",format(Sys.time(), "%d/%m/%Y")), caption = "Os dados são atualizados diariamente.\n Fonte:https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide") + xlab("País") + ylab("Leitos por 1000 habitantes")+
