...
}, gif_file= paste0("./COVID19_SC.gif"),width = 800, height = 600, res = 144, delay=1/15)
...
map_100k <- leaflet(shp_sf_last_data, width = "100%") %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
...
p_lin <- data %>% 
  filter(confirmed > 10) %>% 
  ggplot(aes(x=days_from_10th, y=confirmed, color=city,label=state)) + geom_line(size=1) + xlab("Dias desde 10º caso") + ylab("Casos confirmados")+ theme_minimal()


p_casos_cidade_sc <- p_lin %>% ggplotly() %>% custom_layout(paste("<b>Evolução por cidade</b>"),
                                                            "Em escala linear",
...
p_casos_cidade_sclog <- p_log %>% ggplotly() %>% custom_layout(paste("<b>Evolução por cidade</b>"),
                                                            "Em escala logarítmica",
                                                            paste0("Fonte:Secretaria de Saúde de SC", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y")))
)

p_casos_cidade_sclog

if (prod == TRUE) {api_create(p_casos_cidade_sclog, filename = "covid-sc-citylog", fileopt = "overwrite")}
...
p_casos_cidade_mrlin <- p_lin %>% ggplotly() %>% custom_layout(paste("<b>Evolução por microrregião</b>"),
                                                            "Em escala linear",
                                                            paste0("Fonte:Secretaria de Saúde de SC", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y")))
)

p_casos_cidade_mrlin

if (prod == TRUE) {api_create(p_casos_cidade_mrlin, filename = "covid-sc-mrlin", fileopt = "overwrite")}
...
p_casos_cidade_mrlog <- p_log %>% ggplotly() %>% custom_layout(paste("<b>Evolução por microrregião</b>"),
                                                            "Em escala logarítmica",
                                                            paste0("Fonte:Secretaria de Saúde de SC", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y")))
)

p_casos_cidade_mrlog

if (prod == TRUE) {api_create(p_casos_cidade_mrlog, filename = "covid-sc-mrlog", fileopt = "overwrite")}
```
## Códigos
