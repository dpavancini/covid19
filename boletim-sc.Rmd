...
p_obt_lin <- p_obt %>% ggplotly(dynamicTicks = TRUE, tooltip = c("text"))%>% custom_layout(paste("<b>Evolução de número de óbitos</b>"),
                                                            "Em escala linear",
                                                           paste0("Fonte: Secretaria de Saúde de SC.", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y"))))
...
x_seq <- data_SC_state %>% 
  mutate(
    ...
    y_est_08 = deaths/0.008 # estimativa com taxa de mortalidade basileria (0.0675)
     )
...
p_log <- x_seq %>% 
  ...
  geom_line(aes(x=days_from_10th,y=y_est_15),data=x_seq, color = "grey")  +
  ...
  annotate("text", x = c(last(x_seq$days_from_10th),last(x_seq$days_from_10th),last(x_seq$days_from_10th),last(x_seq$days_from_10th)), 
           y = c(max(x_seq$y_est_40),max(x_seq$y_est_20),max(x_seq$y_est_15),max(x_seq$y_est_10)), 
           label = c("4%","2%","1,5%","1%"), 
           size=3, color = "grey",vjust=-1) + 
  theme(legend.position="bottom") +
  scale_color_discrete(name="")
