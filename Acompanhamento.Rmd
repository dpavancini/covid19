...
p <- dados_selecionados %>% 
  ggplot(aes(x=days_from_first, y=sum_cases, color=labels, linetype=labels,label=labels)) + geom_line(size=1) + 
...
p_1 <- p %>% ggplotly() %>% layout(title=paste("Evolução após 1º caso"), 
...
if (prod == TRUE) {api_create(p_1, filename = "covid-world-1st-plot",overwrite=T)}
```
...
#Create labels
p <- dados_selecionados %>% 
  ggplot(aes(x=days_from_100th, y=sum_cases, color=labels, linetype=labels,label=labels)) + geom_line()+ 
...
p_2 <- p %>% ggplotly() %>% layout(title=paste("Evolução após 100º caso,"), 
                            annotations = list(x = 0.8, y = -0.05, text = paste0("Fonte:ECDC.", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y"))), 
                                          showarrow = F, xref='paper', yref='paper', 
                                          xanchor='left', yanchor='auto', xshift=0, yshift=0,
                                          font=list(size=12, color="grey")))
  ...
  # Upload to chart studio
if (prod == TRUE) {api_create(p_2, filename = "covid-world-100th-plot")}
```
...
p<- dados_selecionados_estat %>% 
  ggplot(aes(x=days_from_100th, y=cases_per_1000, color=labels, linetype=labels, label=labels)) + geom_line()+ 
...
p_3 <- p %>% ggplotly() %>% layout(title=paste("Casos por 1000 habitantes"), 
                            annotations = list(x = 0.8, y = -0.05, text = paste0("Fonte:ECDC.", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y"))), 
                                          showarrow = F, xref='paper', yref='paper', 
                                          xanchor='left', yanchor='auto', xshift=0, yshift=0,
                                          font=list(size=12, color="grey")))

if (prod == TRUE) {api_create(p_3, filename = "covid-world-attack-plot")}
```
...
p_4 <- p_lin %>% ggplotly() %>% layout(title=paste("Evolução por estado (linear)"), 
...
p_5 <- p_log %>% ggplotly() %>% layout(title=paste("Evolução por estado (em log)"), 
                            annotations = list(x = 0.8, y = -0.05, text = paste0("Fonte: SES.", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y"))), 
                                          showarrow = F, xref='paper', yref='paper', 
                                          xanchor='left', yanchor='auto', xshift=0, yshift=0,
                                          font=list(size=12, color="grey")))

if (prod == TRUE) {api_create(p_5, filename = "covid-states-log-plot")}
...
p_lin_1000 <- casos_por_estado %>% 
  filter(totalCases > 10 & state != 'TOTAL' & date < Sys.Date()) %>% 
  ggplot(aes(x=days_from_10th, y=total_cases_pop, color=state,label=state)) + geom_line()+ 
  geom_text(aes(x=days_from_10th,y=total_cases_pop, label = state,color = state), data = dados_rotulos, show.legend = F) +
  xlab("Dias desde 10º caso") + ylab("Casos confirmados")+ theme_minimal()

p_6 <- p_lin_1000 %>% ggplotly() %>% layout(title=paste("Casos por 100mil/hab (linear)"), 
                            annotations = list(x = 0.8, y = -0.05, text = paste0("Fonte: SES.", paste("Atualizado:",format(Sys.time(), "%d/%m/%Y"))), 
                                          showarrow = F, xref='paper', yref='paper', 
                                          xanchor='left', yanchor='auto', xshift=0, yshift=0,
                                          font=list(size=12, color="grey")))

if (prod == TRUE) {api_create(p_6, filename = "covid-states-pop-lin-plot")}
...
p_7 <- p_log_sc %>% ggplotly() %>% layout(title=paste("Casos de SC"), 
...
if (prod == TRUE) {api_create(p_7, filename = "covid-sc-log-plot")}
```
