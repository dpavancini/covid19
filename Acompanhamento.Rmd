...
### Log scale
x_seq <-data_frame(days_from_10th = seq(0,100,1)) %>% 
  mutate(
    y_log_1d = 10 + days_from_10th * 2,
    y_log_2d = 10 + days_from_10th * log2(2),
    y_log_5d = 10 + days_from_10th * log2(5),                                                                                                 y_log_10d = 10 + days_from_10th * log2(10) )      
  
  
p_log <- casos_por_estado %>% 
  filter(totalCases > 10 & state != 'TOTAL' & date < Sys.Date()) %>% 
  ggplot(aes(x=days_from_10th, y=totalCases, color=state,linetype=state,label=state)) + geom_line(size=1)) + scale_y_continuous(trans="pseudo_log") + labs(title = paste("Evolução por estado (em log) - ",format(Sys.time(), "%d/%m/%Y")), caption = "Em escala logaritmica. Apenas estados com mais de 10 casos.Os dados são atualizados diariamente.\n Fonte:https://raw.githubusercontent.com/wcota/covid19br/master/cases-brazil-states.csv") + xlab("Dias desde 10º caso") + ylab("Casos confirmados")+ theme_minimal() 
...
if (prod == TRUE) {api_create(p_lin, filename = "covid-states-pop-lin-plot")}
