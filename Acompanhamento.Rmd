...
read_CityDataCovid <- function() {
  ...
  my_content <- httr::content(tf, as = 'text')
  my_content_from_json <- jsonlite::fromJSON(my_content)
  out<- my_content_from_json$results
  # verify more pages of data and get all
  while(!is.null(my_content_from_json$`next`)){
    url <- my_content_from_json$`next`
    tf <- GET(url)
    my_content <- httr::content(tf, as = 'text')
    my_content_from_json <- jsonlite::fromJSON(my_content)
    results_df_np<- my_content_from_json$results
...
cidade_latlng <- read.csv("municipios.csv")
...
leaflet(last_data) %>% addTiles() %>%
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addCircleMarkers(
    radius = ~sqrt(last_data$confirmed) * 5,
...
```{r}
## attempt to make a graph / map move

## map 
map_reveal <- ggplot(
  shp_sf,
  aes(x = lng, y=lat, group = city,size = confirmed * 50, colour = 'purple',alpha = .25)
  ) +
  geom_point()+
  scale_color_viridis_d() +
  # geom_sf(data = sc_map) +
  labs(x = "Dias desde o primeiro caso", y = "Casos Confirmados") +
  theme(legend.position = "none") +
  transition_manual(date)

# lat/lng works, but whithout map in background
map_reveal <-map_reveal + transition_reveal(date)
map_reveal


# line chart moving.. will be improved--
line_reveal <- ggplot(
  data,
  aes(x = date, y=confirmed, group = city,colour = factor(city))
  ) +
  geom_line()+
  scale_color_viridis_d() +
  labs(x = "Dias desde o primeiro caso", y = "Casos Confirmados") +
...
line_reveal <- line_reveal + transition_reveal(date)
line_reveal
